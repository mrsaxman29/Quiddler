...
server = "192.168.1.151" # SERVER SETTINGS # PIER 192.168.1.213 HOME 192.168.1.151
port = 5555
...
connected_socket, addr = listening_socket.accept()      # connected_socket #
print("Connected to:", addr)

root = Tk()
root.config(bg="green")
root.geometry("1440x780")
root.title('PLAYER 1 (SERVER)')

blank_frame = Frame(root,height=30, width=100, bg="green").grid(row=0, column=0, padx=0, pady=0)
...
def submit():
    ...
    if len(temp_cards) == cards_per_hand:   # IF YOU USED ALL CARDS #
        ...
        card_str = deck[discarded_card[0]].letter
        ...
        connected_socket.sendall(pickle.dumps(data_to_send))
        ...
        print(f'YOU USED ONE TOO MANY CARDS!  THIS IS A {str(cards_per_hand)} CARD HAND - RESETTING HAND')
        words = []
        word_offset = 0
        your_cards_info = {}
        round_score -= points
...
def discard_card():     # if you want to keep draw card, (if deck card not clicked and revealed, and you don't discard the draw card, must shuffle deck so draw card is now in hand
    global turn
    ...
    if len(your_cards_info) == 1:
        ...
        connected_socket.sendall(pickle.dumps(data_to_send))    # SENDING

    else:
        print('YOU CAN ONLY DISCARD 1 CARD PER HAND')  #ADD GUI pop up for user
...
your_cards[10] = Button(root, image=deck[10].frontimage, command=lambda: draw_from_draw_pile())  ##  MAKE NEW FUNCTION: MOVES CARD TO YOUR HAND AREA ON FIRST CLICK
your_cards[10].grid(row=2, column=2, padx=0, pady=25)
...
data = deck[10].letter
connected_socket.sendall(pickle.dumps(data))    # 1ST SEND # DRAW CARD TO CLIENT #
...
while game:         # GAME LOOP #
    ...
    if game_state == 'PLAYING':
        ...
        if not turn:
            ...
            data = pickle.loads(connected_socket.recv(2084*6))   # 1ST RECV # DISCARD CARD FROM CLIENT # BECOMES DRAW CARD
        ...
        if not last_turn:
            ...
            data = pickle.loads(connected_socket.recv(2084 * 6))
            print(f'RECEIVED: {data} FROM CLIENT')
...
# WHEN YOU TRY AND SUBMIT (TOTAL OF) 1 LESS CARD THAN CARDS PER HAND (ILLEGAL))
#   THIS MEANS YOU WOULD HAVE ONLY 1 CARD LEFT TO SPELL WITH
#   AND YOU WOULD NEED AT LEAST 2 CARDS TO MAKE A LEGAL SUBSEQUENT MOVE

# SCORING

##   THREADING ???

##  WORKING ON 2 SEPERATE COMPUTERS

## bgimage.jpg   768×658 pixels    total area 1420x680
